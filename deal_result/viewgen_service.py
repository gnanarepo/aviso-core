...
class CoolerViewGeneratorService(object):
    '''
    New viewgen
    ...
    def __init__(self, perspective=None, hier_asof=None,):
        ...
        self.hier_svcs = {hier_key: CoolerHierarchyService(config, asof=hier_asof) for hier_key, config in hier_config.items()}
    ...
    def get_views(self, raw_record):
        ...
        for dd_key, dd_svcs in self.drilldowns.items():
            ...
            # we can just do the last part here
            # TODO: maybe rework the info portion
        ...
        # TODO: make this more optimal, should be able to do some prechecks like
        # is the dd_key right, etc
    ...
    def try_concat(self, out_dict, split_vals, split_id, node):
        try:
            if out_dict[node]:
                return out_dict[node] + ' / ' + split_vals[split_id] if split_vals[split_id] and split_vals[split_id] not in out_dict[node].split(' / ') else out_dict[node]
            else:
    ...
    def format_output(self, record, disp_map):
        # TODO: I think there's a better way to do this at the record creation level
        # if we have a perspective, we need to funnel down the record outputs
    ...
    def split_unborn(self, record):
        ...
        for dd_key, dd_svcs in self.drilldowns.items():
            ...
            if dd_split is not None:
                ...
                split_dict, split_ids = CoolerSplitterService.dummy_unborn_splits(record, self.unborn_amount_fields[dd_key])
            # given split_ids, makes a reference to hier nodes,
            # then appends any groupby/field info
