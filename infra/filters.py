...
def _sanitize_string(string):
    ...
    set_pattern, set_replace = r"\'(set)\(\[.*?\]\)\'", r"(?<=set\()\[.*?\]"
    if re.findall(set_pattern, string):
        string = re.sub(set_pattern, re.findall(set_replace, string)[0], string)
    list_pattern, list_replace = r"\'\[.*?\]\'",  r"\[.*?\]"
    if re.findall(list_pattern, string):
        string = re.sub(list_pattern, re.findall(list_replace, string)[0], string)
    return string

def contextualize_filter(filt,
                         node,
                         favs,
                         period,
                         ):
    """
    inject app context into filter

    Arguments:
        filt {dict} -- mongodb filter description                               {'opp_id': {'$in': '%(favs)s'}}
        node {str} -- hierarchy node                                            '0050000FLN2C9I2'
        favs {set} -- opp_ids of favorited deals                                {'0060b00000nRtMS', }
    ...
    """
    # puke
    #adding a replace for node to add escape characters for single quotes in a node value
    #ex: "Global#Becky O'Brien#!"
    str_filt = str(filt).replace("%s", "*pers*").replace("%(", "*perb*").replace("%", "*perc*").replace("*pers*", "%s").replace("*perb*", "%(")
    resp = eval(_sanitize_string(str_filt % {'node': node.replace("'","\\'") if node else None, 'favs': favs, 'period': period}))
    if "*perc*" in str_filt:
...
def fetch_user_levels_filters(config,
                              filter_type='mongo',
                              db=None,
                              collection=FILTS_COLL
                              ):
    """
    fetch all the user level filters from the db
    ...
    all_filters = {}
    filters_collection = db[collection] if db else sec_context.tenant_db[collection]
    final_criteria = {"user_level": True, "user": sec_context.login_user_name, "$or": [{"deleted": {"$exists": False}}, {"deleted": False}]}
    filters = filters_collection.find(final_criteria, {'desc': 1, 'name': 1, 'filter_id': 1, "tenant_level":1, "user_level":1, "group_name":1})
    for filt in filters:
        org_filt = deepcopy(filt)
        tenant_level = False if filt.get("user_level") else True
        all_filters[filt['filter_id']] = (filt['name'], parse_filters(filt['desc'], config, filter_type), org_filt['desc'], tenant_level)

    return all_filters


def fetch_filters_by_filter_id(config,
                               filter_type='mongo',
                               db=None,
                               filter_ids=None,
                               collection=FILTS_COLL,
    ...
    """
    fetch the filter details by filter_id from the db
    ...
    criteria = {'filter_id': {"$in": filter_ids}}
    filters = filters_collection.find(criteria, {'desc': 1, 'name': 1, 'filter_id': 1, "tenant_level":1, "user_level":1, "group_name":1})
    for filt in filters:
        org_filt = deepcopy(filt)
        tenant_level = False if filt.get("user_level") else True
        all_filters[filt['filter_id']] = (filt['name'], parse_filters(filt['desc'], config, filter_type, is_pivot_special=is_pivot_special), org_filt['desc'], tenant_level)
...
def fetch_filter_names(filter_ids,
                       db=None,
                       collection=FILTS_COLL
                       ):
    """
    fetch human readable name and ids of filters
    ...
    Returns:
        list -- [{'name': name, 'filter_id': filter_id}]
    """
    filters_collection = db[collection] if db else sec_context.tenant_db[collection]
    if filter_ids:
        criteria = {'filter_id': {'$in': filter_ids}}
        ...
        criteria = {'is_default': True}
    return filters_collection.find(criteria, {'name': 1, 'filter_id': 1, 'is_segment_filter': 1})


def upload_user_level_filter(filter_data, db=None,collection=FILTS_COLL):
    action = filter_data.pop("action", "create")
    filters_collection = db[collection] if db else sec_context.tenant_db[collection]
    errors = []
    if filter_data['name'].strip() == '':
        errors.append({'filter': filter_data, 'error': 'You cannot save an empty name'})
        return errors
    try:
        _valid_filter(filter_data['desc'])
        record = {'name': filter_data['name'],
                  'desc': filter_data['desc'],
                  'user_level': True,
                  'user': sec_context.login_user_name
                  }
        if action == "create":
            user_filter = filters_collection.find({"name": record['name'], "user": sec_context.login_user_name})
            if user_filter.count() > 0:
                if user_filter[0] and user_filter[0].get("deleted"):
                    filters_collection.update_one({"filter_id": user_filter[0]['filter_id'], "user": sec_context.login_user_name},
                                              {'$set': record, '$unset': {"deleted": 1, "in_grid": 1}})
                else:
                    return [{"filter": filter_data, "error": "Filter Already Exists with the name `%s`"%record['name']}]
            else:
                record["filter_id"] = str(uuid.uuid4())
                filters_collection.insert(record)
        else:
...
def upload_filter(name,
    ...
    """
    upload a filter to db

    Arguments:
        name {str} -- display name of filter                                    'Open Deals'
        desc {list} -- our insane filter language format                        [{'op': 'has', 'key': 'amt'}]

    Keyword Arguments:
        filter_id {str} -- unique id for filter (default: {None})
        is_default {bool} -- makes filter default (default: {False})
        is_open {bool} -- makes filter open (default: {False})
        in_grid {bool} -- make filter appear in deals grid (default: {True})
        db {object} -- instance of tenant_db (default: {None})
                       if None, will create one
    """
    filters_collection = db[collection] if db else sec_context.tenant_db[collection]
    audit_collection = db[AUDIT_COLL] if db else sec_context.tenant_db[AUDIT_COLL]
    _valid_filter(desc)
    ...
    record = {'name': name,
              ...
              'is_segment_filter': is_segment_filter}
    if custom_limit is not None:
        record['custom_limit'] = int(custom_limit)

    filters_collection.update_one({'filter_id': filter_id}, {'$set': record}, upsert=True)

    # audit = {'service': 'deal_svc',
    #          'action': 'save filter',
    #          'user': sec_context.login_user_name,
...
def delete_filter(filter_id,
    ...
    Arguments:
        filter_id {str} -- unique id of filter                                  'id1'
    ...
    audit_collection = db[AUDIT_COLL] if db else sec_context.tenant_db[AUDIT_COLL]
    if user_level:
        record = filters_collection.find_one_and_update({'filter_id': filter_id, 'user_level': True, 'user': sec_context.login_user_name},
                                                        {'$set': {'deleted': True,
                                                                  'in_grid': False}})
    else:
        record = filters_collection.find_one_and_update({'filter_id': filter_id, "$or": [{"user_level": {"$exists": False}}, {"user_level": False}]},
                                                    {'$set': {'deleted': True,
                                                              'in_grid': False}})
    # audit = {'service': 'deal_svc',
...
def negate_filter(desc):
    """
    negate a filter

    Arguments:
        desc {list} -- filter description to negate                             [{'op': 'has', 'key': 'amt'}]

    Returns:
        list -- negated filter
    """
    new_filt = [clause.copy() for clause in desc]
    for clause in new_filt:
        clause['negate'] = not clause.pop('negate', False)
    return new_filt
...
def passes_new_filter(deal_data, fltr_desc=None, favs_set=None, dlf_svc=None):
