...
import traceback
import logging
from gbm_apis.domainmodel import UIPIterator
...
class CacheOptions(object):
    def __init__(self, src_uip_ds, source_id_list,
                 start_size, to_process):
        self.src_uip_ds = src_uip_ds
        self.source_id_list = source_id_list
        self.is_full = source_id_list == []
    ...
    def time_series_reduce(self, out_rec, uip_obj_list, config):
        ...
        for uip_obj in sorted(uip_obj_list, key=lambda x: x.created_date):
            try:
                ...
                logger.warn("WARNING: field '%s' missing for id ='%s', during time series reduce" % (
                    ts_fld, uip_obj.ID))
                continue
            try:
