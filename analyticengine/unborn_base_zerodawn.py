...
class UnbornBaseModelZeroDawn:
    ...
    def __init__(self, field_map, config, time_horizon, dimensions):
        super(UnbornBaseModelZeroDawn, self).__init__(field_map, config, time_horizon, dimensions)
        ...
        try:
            self.std_mult = self.forecast_params['bands_scheme']['std_mults'][0]
        except (KeyError, IndexError):
            self.std_mult = 1.2815515655446004
        ...
        # this is some new init stuff to handle zero dawn specific stuff
        self.dd_hist_aggs = defaultdict(lambda: defaultdict(TSAggregator))
        self.dd_actv_aggs = defaultdict(lambda: defaultdict(TSAggregator))
        self.drilldown_keys = self.viewgen_svc.drilldown_keys
        self.drilldown_flds = self.viewgen_svc.drilldown_fields
        self.level_groupby_flds = self.viewgen_svc.level_groupby_fields
        self.all_dim_flds = {dd_key: dd_flds + self.model_dim_flds
                             for dd_key, dd_flds in self.drilldown_flds.items()}
    ...
    def setup_period_infos(self):
        ...
        for time_point in self.history_timeline_details:
            ...
            # and the rest are additional pairs denoting pois for by_period results
    ...
    def _is_historic_existing_unborn(self, created_date, real_created_date, terminal_date, terminal_fate, hist_begin,
        ...
        created_since_begin = hist_as_of < created_date <= hist_begin
        won_in_period = hist_begin <= terminal_date <= hist_horizon

        if statute_of_limitations is None:
            within_statute = True
        else:
            within_statute = real_created_date < hist_begin + statute_of_limitations

        return (created_since_begin and within_statute and won_in_period)
    ...
    def process_observation(self, active_record, observation):
        ...
        for obs in record.observations:
            ...
            # One split rec per split.
            split_recs = self.get_splits(record)
            for split_rec in split_recs:
                (drilldown_index, splt_amt, path_tuple) = split_rec
                if obs.category in ['HE', 'HN', 'HPN']:  # If is Historic Unborn -> Go into Hist Aggs
                    drilldown_vals = path_tuple + tuple([obs.period_idx, obs.category == 'HE'])
                    ...
                    self.dd_hist_aggs[drilldown_index][drilldown_vals].add('amount',
                                                                           (record.created_date, splt_amt))
                    self.dd_hist_aggs[drilldown_index][drilldown_vals].add('count',
                                                                           (record.created_date, 1))
                else:
                    drilldown_vals = path_tuple
                    self.dd_actv_aggs[drilldown_index][drilldown_vals].add('amount',
                                                                           (self.th.as_ofF, splt_amt))

                    self.dd_actv_aggs[drilldown_index][drilldown_vals].add('count',
                                                                           (self.th.as_ofF, 1))

    def get_total_results(self):
        result = {}
        for key in self.viewgen_svc.drilldown_keys:
            dd_results = self.get_total_results_atomic(key)
            result.update(dd_results)
        return result


    def calculate_result_for_drilldowns(self, res, prefix, df, drilldown_key, dd_and_dim_flds, drilldown_flds):
        ...
        # need second case cause you can't groupby empty
        if not drilldown_flds:
            tmp_df = pd.DataFrame()
            logger.info("Unborn Model Top Level Summary:\n%s", agg_df.to_string())
            tmp_df.loc['summary', 'fcst_mean'] = np.sum(agg_df['contrib_amt'])
            tmp_df.loc['summary', 'fcst_std'] = self.ssd(agg_df['amount'])
            agg_df = tmp_df
        else:
        ...
        for grp_vals, grp_res in agg_df.iterrows():
            # TODO: add an escape for 'nullval'
            # to cover for hair on top, we wanna skip any time your last value is null
            lastval = grp_vals[-1] if isinstance(grp_vals, tuple) else grp_vals
            if lastval == 'nullval':
                continue
            ...
            if math.isnan(fcst_std):
                fcst_std = fcst_mean
            lower_band = max(0, fcst_mean - self.std_mult * fcst_std)
            upper_band = fcst_mean + self.std_mult * fcst_std

            res_key = self.viewgen_svc.format_unborn_node(drilldown_key, drilldown_flds, grp_vals)

            all_empty = lower_band == 0 and fcst_mean == 0 and upper_band == 0
            if not drilldown_flds or not all_empty:
                res[res_key] = {
                    prefix + 'down': lower_band,
                    prefix + 'value': fcst_mean,
                    prefix + 'up': upper_band,
                }
            if not drilldown_flds and drilldown_key == self.drilldown_keys[0]:
                res['.'] = {
                    prefix + 'down': lower_band,
                    prefix + 'value': fcst_mean,
                    prefix + 'up': upper_band,
    ...
    def get_total_results_atomic(self, dd_index):
        new_results = {}
        existing_results = {}
        results = {}

        hist_aggs = self.dd_hist_aggs[dd_index]
        actv_aggs = self.dd_actv_aggs[dd_index]
        drilldown_fields = self.drilldown_flds[dd_index]
        all_dim_fields = self.all_dim_flds[dd_index]
        level_groupby_fields = self.level_groupby_flds[dd_index]

        for agg in hist_aggs.values():
            agg.seal_for_query()

        # The index of the period field in the dim_vals.
        prd_idx = len(drilldown_fields)
        ...
        if hist_df.shape == (0, 0):
            ...
            hist_df = pd.DataFrame([["."] * len(all_dim_fields) + [0, 0, 0, 0]])
        hist_df.columns = all_dim_fields + ('bop_amt', 'bop_cnt', 'amount', 'count')
        ...
        if actv_df.shape == (0, 0):
            ...
            if self.redist_scheme.get('name', 'none') != 'none':
                raise Exception(msg)
            actv_df = pd.DataFrame([["."] * len(all_dim_fields) + [0, 0, 0, 0]])
        actv_df.columns = drilldown_fields + ('amount', 'count')
        actv_df.sort_index(inplace=True)

        #we need to modify the distribution of the unborn if we're a future run
        if self.model_scheme['name'] in ['precomputed_mults', 'precomp_with_regional']:
            # TODO: handle secondary drilldowns.
            # TODO: double check the try except block, and come up with better solution
            if self.produce_existing_results:
                try:
                    ...
                    logger.warning("hist_df and hist_existing_df amounts are zero")
                    self.runtime_params['unborn_np'] = self.runtime_params['scaled_ubf'] / 2.0
                    self.runtime_params['unborn_ep'] = self.runtime_params['scaled_ubf'] / 2.0
        ...
        hist_df = self.apply_model(hist_df, actv_df, dd_index).reset_index()
        if self.produce_existing_results:
            hist_existing_df = self.apply_model(hist_existing_df, actv_df, dd_index, newpipe_part=False).reset_index()


        logger.warning('DRILLS {}'.format(level_groupby_fields))
        for dim_flds_key, drilldown_flds in level_groupby_fields.items():
            self.calculate_result_for_drilldowns(new_results, '', hist_df, dd_index,
                                                 drilldown_flds + self.model_dim_flds, drilldown_flds)
            if self.produce_existing_results:
                self.calculate_result_for_drilldowns(existing_results, 'existing_', hist_existing_df,
                                                     dd_index, drilldown_flds + self.model_dim_flds, drilldown_flds)
        if self.produce_existing_results:
    ...
    def apply_model(self, hist_df, actv_df, dd_index, newpipe_part=True):
        '''
        ...
        '''
        hist_df = self.redistribute_amounts(hist_df, actv_df, dd_index)
        ...
        if model_scheme == 'mean':
            ...
            region_fld = self.drilldown_flds[dd_index][0]
            prd_fld = self.model_dim_flds[0]
    ...
    def redistribute_amounts(self, hist_df, actv_df, dd_index):
        '''
        ...
        hist_df = hist_df.set_index(list(self.drilldown_flds[dd_index]))
        actv_df = actv_df.set_index(list(self.drilldown_flds[dd_index]))
        ...
        if not redist_hier:
            ...
            for x in redist_hier:
                if len(x.split('~')) > 1:
                    logger.warning("WARNING: Unborn Model found tilde in field name for redist_hierarchy")
        redist_hier_available = [x for x in redist_hier if x in self.drilldown_flds[dd_index]]
        if len(redist_hier_available) != len(redist_hier):
            redist_hier = redist_hier_available
            logger.warning("WARNING: Unborn model will use %s for redistribution", redist_hier)
        ...
        actv_df = UnbornBaseModelZeroDawn.remove_forbidden_rows(actv_df, no_redist_to)
        new_hist_df = UnbornBaseModelZeroDawn.remove_forbidden_rows(hist_df, no_redist_to)
        ...
        hist_df = self.try_reset_index(hist_df).set_index(list(self.all_dim_flds[dd_index]))
        untouchable_df = untouchable_df.reset_index().set_index(list(self.all_dim_flds[dd_index]))
        if remerged:
            if self.debug_dims:
            ...
            try:
                remerged.index.names = list(self.all_dim_flds[dd_index])
            except:
                logger.info('remerged df has tuples in index for %s drilldown', str(dd_index))
                remerged.index = pd.MultiIndex.from_tuples(remerged.index)
                remerged.index.names = list(self.all_dim_flds[dd_index])
        ...
        return self.try_reset_index(hist_df)
    ...
    def get_splits(self, record):
        rec = {
            fld: record.get_dimension_valueF(fld, self.th, ubf_accounting=True)
            for fld in self.viewgen_svc.unborn_fields}
        # TODO: Change this to pass in the amount.
        try:
            blah = self.viewgen_svc.split_unborn(rec)
        except Exception as e:
            logger.error('viewgen unborn - rec: {}, error: {}'.format(rec, e))
        return blah

    def ssd(self, nparr):
        return mathUtils.ssd_fc(nparr, self.num_cycles)

    def try_reset_index(self, df):
        # with new hierarchy, sometimes we can have empty levels, which messes with reindex, so we gotta do some jank stuff
        try:
            return df.reset_index()
        except IndexError:
            bad_indices = [idx_name for idx_name, idx_vals in zip(df.index.names, df.index.levels) if not idx_vals.nunique()]
            logger.warning(f'Trying to reset index with empty hierarchy levels {bad_indices}')
            for index in bad_indices:
                df.index = df.index.droplevel(level=index)
